...
def reward_request_submitted(request, *args, **kwargs):
        ...
        rr= rewardRequest(name=context['person'],reward=context['chore'],date=context['date'])
        rr.save()
...
#    reward = []
    for i in rewards.objects.all():
        ...
        form.fields['reward'].widget.choices.append((i.name.replace('&&', '_'),i.name.replace('&&', '_')+': '+str(i.pointValue)+' points'))
